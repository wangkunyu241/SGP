...
class VITAttentionRollout:
    def __init__(self, model, head_fusion="mean", discard_ratio=0.8):
        self.model = model
...
def clean_accuracy_visual(model: nn.Module,
                   x: torch.Tensor,
                   y: torch.Tensor,
                   path: list = [],
                   batch_size: int = 100,
                   corruption_type: str = 'error', 
                   adaptation_type: str = 'error',
                   device: torch.device = None):
    if device is None:
        device = x.device
    acc = 0.
    ...
    attention_rollout = VITAttentionRollout(model.model.vit)
    with torch.no_grad():
        for counter in range(500):
            x_curr = x[counter:(counter + 1)].to(device)
            mask = attention_rollout(x_curr)
            save_dir = "/output/attention_map/{}/{}".format(adaptation_type, corruption_type)
            os.makedirs(save_dir, exist_ok=True)
            attn_name = "{}/{}.png".format(save_dir, counter)
            # clean_name = "{}/{}_clean.png".format(save_dir, counter)
            
            img = Image.open(path[counter])
            img = img.resize((224, 224))
            np_img = np.array(img)[:, :, ::-1]
            # cv2.imwrite(clean_name, np_img)

            mask_resized = cv2.resize(mask, (np_img.shape[1], np_img.shape[0]))
            masked_img = show_mask_on_image(np_img, mask_resized)
            cv2.imwrite(attn_name, masked_img)
    attention_rollout.remove_hooks()

    return acc.item() / x.shape[0]

def clean_accuracy_FT(model: nn.Module,
                   x: torch.Tensor,
                   y: torch.Tensor,
                   batch_size: int = 100,
                   device: torch.device = None):
...
def clean_accuracy_dpcore(model: nn.Module,
                   x: torch.Tensor,
                   y: torch.Tensor,
                   batch_size: int = 100,
                   device: torch.device = None):
