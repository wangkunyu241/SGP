...
class CustomDatasetFolder(VisionDataset):
    ...
    def safe_loader(self, path, retries=3):
        for _ in range(retries):
            try:
                return self.loader(path)
            except (OSError, IOError, Image.DecompressionBombError) as e:
                print(f"[WARN] Skipped corrupted image {path}: {e}")
                return None
        return None

    def __getitem__(self, index):
        path, target = self.samples[index]
        sample = self.safe_loader(path)
        
        if sample is None:
            # Skip corrupted image and recursively get the next valid sample
