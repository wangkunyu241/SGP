...
def is_frozen_layer(name):
    filter_keywords = ['blocks.9', 'blocks.10', 'blocks.11', 'norm.']
...
class Pruning(nn.Module):
    def __init__(self, model, optimizer, lamda_std, lamda_align, e_margin, rate, lamda_sparse, threshold, lr_mask, steps=1):
        ...
        self.stats_file = "./source_stats/cifar10_stats_pruning.pt"
    ...
    def _register_pruning_hooks(self):
        """给前8层的 attention projection 注册 pre-hook"""
        for i in range(self.num_prune_layers):
            # 获取对应的模块 (timm 中是 blocks[i].attn.proj)
    ...
    def _load_or_compute_stats(self, num_samples=10000):
        ...
        class RawCIFAR10Dataset(Dataset):
            def __init__(self, root):
                ...
                self.data = np.stack([np.array(img) for img, _ in base])
        ...
        dataset = RawCIFAR10Dataset('your_cifar10_dir')
        loader = DataLoader(dataset, batch_size=64, shuffle=True, num_workers=4)

        features_list = []
        count = 0
        entropy_threshold = math.log(10)/2 - 1
