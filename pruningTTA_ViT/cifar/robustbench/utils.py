...
#     num_correct = torch.tensor(0., device=device)
#     n_samples = x.shape[0]

#     with torch.inference_mode():
#         for i in range(0, n_samples, batch_size):
#             x_batch = x[i : i + batch_size].to(device, non_blocking=True)
#             y_batch = y[i : i + batch_size].to(device, non_blocking=True)
            
#             output = model(x_batch)
#             pred = output.argmax(dim=1)
#             num_correct += (pred == y_batch).sum()

#     return num_correct.item() / n_samples
